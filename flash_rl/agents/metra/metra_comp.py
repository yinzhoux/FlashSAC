...
def compute_metra_reward(
    current_features: torch.Tensor, next_features: torch.Tensor, skills: torch.Tensor, skill_type: str = "continuous"
):
    alignment, _ = compute_metra_intrinsic_reward(current_features, next_features, skills, skill_type)
    return alignment
...
def gen_optimizer(
    ...
    if scheduler_type == "cosine":
        ...
        scale_scheduler = warmup_cosine_decay_scheduler(init_value, peak, end_value, warmup_steps, decay_steps)
        optimizer = optim.Adam(params=parameters, lr=peak, fused=use_fused)
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda step: scale_scheduler(step) / peak)
        return optimizer, scheduler

    elif scheduler_type == "constant":
        optimizer = optim.Adam(parameters, lr=peak, fused=use_fused)

        scheduler = torch.optim.lr_scheduler.ConstantLR(optimizer=optimizer, factor=1.0)
