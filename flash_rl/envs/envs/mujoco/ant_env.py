...
class AntEnv(MujocoTrait, mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self,
        ...
        mujoco_env.MujocoEnv.__init__(self, model_path, 5, observation_space=None)

        init_obs, _ = self.reset()
        high = np.full(init_obs.shape[1], np.inf, dtype=np.float32)
        low = -high
    ...
    def step(self, a, render=False):
        ...
        return ob, reward, False, done, info
